...
-- Second super admin (dev only). Same rules as above: the role goes in
-- raw_app_meta_data, and the token columns must be '' rather than NULL.
-- Rotate this password before the seed is ever run anywhere but local Docker.
insert into auth.users (
  instance_id, id, aud, role, email, encrypted_password,
  email_confirmed_at, created_at, updated_at,
  raw_user_meta_data, raw_app_meta_data, is_super_admin,
  ...
  'authenticated',
  'authenticated',
  'venkatatmakuri75@gmail.com',
  extensions.crypt('Venkat@5421', extensions.gen_salt('bf')),
  now(), now(), now(),
  '{"full_name": "Venkat Atmakuri"}'::jsonb,
  '{"provider": "email", "providers": ["email"], "role": "super_admin"}'::jsonb,
  false,
  '', '', '', '', '', '', '', ''
) on conflict (id) do nothing;
...
insert into auth.identities (
  ...
  'aaaaaaaa-0000-0000-0000-000000000002',
  'aaaaaaaa-0000-0000-0000-000000000002',
  jsonb_build_object('sub', 'aaaaaaaa-0000-0000-0000-000000000002', 'email', 'venkatatmakuri75@gmail.com'),
  'email',
  now(), now(), now()
