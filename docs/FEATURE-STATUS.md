...
**Codebase cleanliness:** a scan for `TODO`, `FIXME`, `HACK`, `not implemented` and `coming soon` across `apps/*` and `packages/*` previously returned one hit — the disabled Customers "Deactivate" action. Fixed 2026-08-15 (`PATCH /api/admin/customers/:id`, migration `0019`); the scan now returns zero hits.
...
| **Wishlist / favourites**       | ✅     | [SPEC-10](specs/SPEC-10-wishlist.md) P0 in full — `wishlist_items` (0017), `/wishlist` API, `WishlistContext`, heart control on all 6 required surfaces, dedicated `/wishlist` page with Move to Cart (R7) and graceful discontinued-item handling |
...
| **Multiple bookings per slot** | ✅     | `branches.capacity` (migration `0018`, default 1), admin-editable in the Branches panel; enforced by the `appointments_enforce_capacity` trigger (advisory-lock + count, replacing the old capacity-1-only unique index), proven under real concurrent load by a 5-racer/capacity-2 e2e test |
...
| **Customers**         | ✅     | Full CRUD-equivalent: list, view, and a real Deactivate/Reactivate action that bans/unbans the linked auth user (not just a display flag) — `PATCH /api/admin/customers/:id`, migration `0019`                                                                                              |
...
| **`generateMetadata`**                                         | 🟡     | Now on `shop`, `category/[slug]`, `product/[slug]` and `search` (per-product `<title>`, description, canonical, OG tags including image; search gets a query-specific title). Still missing on home and the static content pages |
...
| **Server-side rendering**                                      | 🟡     | `shop`, `category/[slug]`, `product/[slug]`, `search`, and the home page's `FeaturedCollection`/`FeaturedProducts`/`TrendingNow` sections are all Server Components (Sprints 5–6). `category/[slug]` was already SSR but read Supabase directly — now goes through `publicApi()` like the rest, closing the last direct-Supabase-read page. The rest of the storefront (auth pages, cart/checkout, account pages) remains `'use client'` by design — see SPEC-03's non-goals |
...
| **API migration — web reads**      | 🟡     | Every catalogue page (`shop`, `category/[slug]`, `product/[slug]`, `search`, home) is off direct Supabase reads as of Sprint 6. **9 files remain** (verified by grep for `createBrowserSupabase`/`createServerSupabase`), all auth/account/cart/order pages — `login`, `signup`, `forgot-password`, `reset-password`, `profile`, `cart`, `order-confirmation/[orderId]`, `orders/[id]/tracking`, `branch-locator` — matching SPEC-03's own non-goal of leaving interactive, non-SEO pages alone. `SearchAutocomplete.jsx` (header typeahead) is a deliberate, separate exception per its own file comment |
