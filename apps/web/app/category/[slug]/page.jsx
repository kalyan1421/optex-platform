...
export default async function CategoryPage({ params }) {
  ...
  const { data: category } = await supabase
    .from('categories')
    // `categories` has no description column (migration 0001) — selecting it
    // made PostgREST error, so `category` came back null and every category
    // page hit notFound(). The heading below already falls back to a product
    // count when there is no description.
    .select('id, name, slug')
    .eq('slug', params.slug)
    .maybeSingle();
