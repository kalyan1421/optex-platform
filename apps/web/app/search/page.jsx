...
function SearchInner() {
  ...
  // Stale-response guard: a slow early query must not overwrite a newer one.
  ...
  // Run whenever the URL query changes (deep link, back button, submit).
  useEffect(() => {
    setInputValue(q);
    runSearch(q);
  }, [q, runSearch]);

  // Instant search: results follow typing after a short pause, without
  // pushing a history entry per keystroke. Skipped while the box still
  // matches the URL so this does not double-run alongside the effect above.
  useEffect(() => {
    const trimmed = inputValue.trim();
    if (trimmed === q.trim()) return undefined;
      ...
      <section className="bg-[#2A3182] py-12 sm:py-16">
        <div className="site-container">
          ...
          {error && (
            <div
              role="alert"
              className="mt-4 flex max-w-2xl items-center justify-between gap-4 rounded-2xl bg-red-50 px-5 py-3.5 text-[13px] font-medium text-red-700"
            >
              <span>{error}</span>
              <button
                type="button"
                onClick={() => runSearch(q)}
                className="shrink-0 rounded-full bg-red-600 px-4 py-1.5 text-[12px] font-bold text-white hover:bg-red-700"
              >
                Retry
              </button>
            </div>
