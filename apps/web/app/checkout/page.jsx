      ...
      <div className="site-container">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
          ...
          <div className="flex flex-1 flex-col gap-4">
            ...
            <div
              className={`rounded-2xl border border-gray-100 bg-white sm:rounded-3xl ${activeStep === 1 ? 'shadow-[0_8px_30px_rgb(0,0,0,0.06)]' : 'shadow-sm'}`}
            >
              <div
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full text-[13px] font-bold ${activeStep === 1 ? 'bg-[#111827] text-white' : 'border border-gray-200 bg-white text-gray-400'}`}
                  >
                    1
                  </div>
                  <div className="flex items-center gap-2 sm:gap-2.5">
                    <TruckIcon />
                    <h3 className="text-[15px] font-bold text-[#2A3182] sm:text-[16px]">
                      Shipping Address
                    </h3>
                  </div>
                </div>
            ...
            <div
              className={`rounded-2xl border border-gray-100 bg-white sm:rounded-3xl ${activeStep === 2 ? 'shadow-[0_8px_30px_rgb(0,0,0,0.06)]' : 'shadow-sm'}`}
            >
              <div
                className="flex cursor-pointer items-center justify-between p-5 sm:p-6"
                onClick={() => setActiveStep(2)}
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full text-[13px] font-bold ${activeStep === 2 ? 'bg-[#111827] text-white' : 'border border-gray-200 bg-white text-gray-400'}`}
                  >
                    2
                  </div>
                  <div className="flex items-center gap-2 sm:gap-2.5">
                ...
                <div className="px-5 pb-6 sm:px-6 sm:pb-8">
                  <div className="border-t border-gray-100 pt-6">
                    <div className="mb-8 flex flex-col gap-3">
                      <label className="flex cursor-pointer items-start gap-4 rounded-xl border-2 border-[#2A3182] bg-blue-50/30 p-4">
                        <input
                          type="radio"
                          name="delivery"
                          defaultChecked
                        ...
                        />
                        <div>
                          <p className="text-[14px] font-bold text-[#2A3182]">
                            Standard Delivery — {formatKes(DELIVERY_FEE_KES)}
                          </p>
            ...
            {/* Step 3 — Payment */}
            <div
              className={`rounded-2xl border border-gray-100 bg-white sm:rounded-3xl ${activeStep === 3 ? 'shadow-[0_8px_30px_rgb(0,0,0,0.06)]' : 'shadow-sm'}`}
            >
              <div
                className="flex cursor-pointer items-center justify-between p-5 sm:p-6"
                onClick={() => setActiveStep(3)}
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full text-[13px] font-bold ${activeStep === 3 ? 'bg-[#111827] text-white' : 'border border-gray-200 bg-white text-gray-400'}`}
                  >
                    3
                  </div>
                  <div className="flex items-center gap-2 sm:gap-2.5">
                  ...
                  <div className="border-t border-gray-100 pt-6">
                    <div className="flex flex-col gap-3">
                      {PAYMENT_METHODS.map((pm) => (
                        <label
                          key={pm.id}
                          className={`flex cursor-pointer items-start gap-4 rounded-xl border-2 p-4 transition-colors ${paymentMethod === pm.value ? 'border-[#2A3182] bg-blue-50/30' : 'border-gray-200 hover:border-gray-300'}`}
                        >
                          <input
                            type="radio"
                            name="payment"
                          ...
                          <div>
                            ...
                            <p className="mt-0.5 text-[12px] text-gray-500">{pm.description}</p>
                          </div>
                        </label>
                      ))}
                    </div>
          ...
          <div className="w-full flex-shrink-0 lg:w-[380px]">
            <div className="sticky top-28 rounded-2xl bg-white p-6 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.1)] sm:rounded-[32px] sm:p-8">
              <h2 className="mb-6 text-[20px] font-bold text-[#2A3182] sm:mb-8 sm:text-[24px]">
                Order Summary
              </h2>
                  ...
                  items.map((item) => (
                    <div key={item.id} className="flex items-center gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="mb-1 truncate text-[14px] font-medium leading-tight text-[#2A3182] sm:text-[15px]">
                          {item.title}
                        </h4>
              ...
              <div className="mb-8 flex items-end justify-between border-t border-gray-100 pt-6">
                ...
                <div className="flex items-center gap-1">
                  <span className="mb-0.5 text-[12px] font-bold uppercase tracking-tighter text-gray-900">
                    KSH.
                  </span>
                  <span className="text-[26px] font-black leading-none text-[#2A3182]">
                    {total.toFixed(2)}
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="mb-6 flex w-full items-center justify-center gap-2 rounded-full bg-[#c1272d] py-4 text-[15px] font-bold text-white shadow-lg shadow-red-500/30 transition-colors hover:bg-red-800 disabled:cursor-not-allowed disabled:opacity-60"
              >
