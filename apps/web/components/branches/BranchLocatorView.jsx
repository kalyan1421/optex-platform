...
import { NAIROBI_LAT, NAIROBI_LNG, buildMapUrl } from '../../lib/maps';
...
// ── View ─────────────────────────────────────────────────────────────────────
/**
 * Interactive half of the branch locator (audit F-13, F-18).
 *
 * Branches arrive as a prop from the Server Component in
 * `app/branch-locator/page.jsx`. This file used to fetch them itself, in a
 * `useEffect`, straight from PostgREST with the anon key — which meant a
 * crawler asking for "opticians in Nairobi" received an empty page. Branch
 * addresses are the single most SEO-relevant content an optician has, and they
 * were invisible.
 *
 * What stays client-side is only what genuinely needs to: which branch is
 * selected, and scrolling the map into view on mobile.
 */
export default function BranchLocatorView({ branches = [] }) {
  const [selectedBranch, setSelectedBranch] = useState(branches[0] ?? null);
  const mapRef = useRef(null);
  // Server-rendered, so the list is present on first paint — there is no
  // loading state left to model.
  const loading = false;
