...
- [x] **Risk-tiered test pass** · [TEST-PLAN.md](docs/TEST-PLAN.md) — Tier 1, Tier 2 and Tier 3 all closed. Tier 1 (money/auth/data-isolation/concurrency): appointments (12 tests, found and fixed a live double-booking race — migration `0014`), order history/tracking (10 tests, protects two already-fixed bugs), prescriptions (10 tests, found the local storage container was down), authenticated checkout (3 Playwright tests, to the M-Pesa-unconfigured boundary). Tier 2 (core UX): cart line items + promo codes (18 tests, money math asserted to the cent), reviews (10 tests: moderation queue, duplicate guard, aggregate rating). Tier 3: `apps/admin` had zero test infrastructure — built a Playwright project with a real cookie-session admin fixture (`middleware.ts` checks a Supabase SSR cookie, not a bearer token), then closed all four originally-scoped candidates — Orders (7 tests, SPEC-06 R3 direct-cancel flow), Products (5 tests, found a real gap: no file picker anywhere despite a fully-built upload endpoint), Payments (2 tests, manual reconcile), Reviews moderation (5 tests, caught the optimistic-update race live — an early draft checked the DB before the PATCH had actually landed). 34→96 API tests, 10→13 web Playwright, 0→19 admin Playwright, all green at least twice in a row. Confirmed the `customers`-row-deleted-but-`auth.users`-row-not pattern is systemic across ~7 Jest/Playwright fixture files, not a one-off — logged as backlog, not fixed inline. Remaining: the full paid-checkout path (blocked on payment-provider sandbox creds) and the two hygiene/product decisions above
