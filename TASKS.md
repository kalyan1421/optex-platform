...
- [x] **Customer-initiated order cancel** · **re-estimated 2 pts → ~7, actual ~8**. It is not a status flip: CLIENT-ANSWERS B5 and [SPEC-06](docs/specs/SPEC-06-order-lifecycle.md) specify a request/approval workflow with admin-set thresholds, notifications and a paid-order acknowledgement. All P0 requirements (R1–R7) shipped; R8–R10 are P1 and tracked below.
  ...
  - [x] **R3 remainder: direct admin cancel without a request** · the phone-call path. A dedicated `PATCH /admin/orders/:id/cancel` (`CancellationService.adminCancel`) gets the same protections as the request-based approval — the R5 paid-order acknowledgement gate, a customer notification, never a refund — and refuses when a pending request already exists for the order rather than silently overriding it. The generic `PATCH /admin/orders/:id/status` can no longer set `cancelled` at all: it had neither the acknowledgement gate nor a notification, and was reachable from the admin Orders screen's plain status dropdown with no warning on a paid order. Cancellation now has exactly one path in, not two
  ...
  - [x] **R5 paid-order acknowledgement** · the API refuses to approve a paid order without `acknowledgePaid`, and the admin screen makes it a dialog rather than a second click. Money already taken on a now-cancelled order is also listed on the Payments screen, so it survives the click that dismissed the dialog. Applies identically to the direct-cancel phone-call path
...
- [x] **Risk-tiered test pass** · [TEST-PLAN.md](docs/TEST-PLAN.md) — Tier 1 and Tier 2 both closed; Tier 3 infrastructure built, Orders and Products closed. Tier 1 (money/auth/data-isolation/concurrency): appointments (12 tests, found and fixed a live double-booking race — migration `0014`), order history/tracking (10 tests, protects two already-fixed bugs), prescriptions (10 tests, found the local storage container was down), authenticated checkout (3 Playwright tests, to the M-Pesa-unconfigured boundary). Tier 2 (core UX): cart line items + promo codes (18 tests, money math asserted to the cent, found and fixed two test-fixture bugs of its own — an FK delete-ordering issue and a PostgREST batch-insert quirk), reviews (10 tests: moderation queue, duplicate guard, aggregate rating). Tier 3: `apps/admin` had zero test infrastructure — built a Playwright project with a real cookie-session admin fixture (`middleware.ts` checks a Supabase SSR cookie, not a bearer token), found and fixed a 144-account auth-user leak in the teardown itself (`listUsers()` paginates at 50/page), then closed Orders — status transitions + the SPEC-06 R3 direct-cancel flow (7 tests) — and Products — CRUD + image upload (5 tests). Also found a real product gap while testing Products: `Products.tsx` has no file picker at all, just an "Image URL" text field, even though the multipart upload endpoint (`POST /products/:id/images`) is fully built and typed — needs a product decision, wire it up or drop it. 34→96 API tests, 10→13 web Playwright, 0→12 admin Playwright, all green three times in a row. Payments/Reviews-moderation (admin) and the full paid-checkout path (blocked on payment-provider sandbox creds) queued
