...
- [x] **Risk-tiered test pass** · [TEST-PLAN.md](docs/TEST-PLAN.md) — Tier 1 and Tier 2 both closed. Tier 1 (money/auth/data-isolation/concurrency): appointments (12 tests, found and fixed a live double-booking race — migration `0014`), order history/tracking (10 tests, protects two already-fixed bugs), prescriptions (10 tests, found the local storage container was down), authenticated checkout (3 Playwright tests, to the M-Pesa-unconfigured boundary). Tier 2 (core UX): cart line items + promo codes (18 tests, money math asserted to the cent, found and fixed two test-fixture bugs of its own — an FK delete-ordering issue and a PostgREST batch-insert quirk), reviews (10 tests: moderation queue, duplicate guard, aggregate rating). 34→96 API tests, 10→13 Playwright, all green twice in a row. Tier 3 (`apps/admin` — zero tests, needs its own Playwright project) and the full paid-checkout path (blocked on payment-provider sandbox creds) queued
